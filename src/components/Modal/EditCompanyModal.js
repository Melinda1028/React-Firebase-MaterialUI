...
const EditCompanyModal = ({ open, onClose, data }: Props) => {
  const classes = useStyles();
  //Form State
  const [name, setName] = useState("");
  const [nameState, setNameState] = useState("success");
  const [credits, setCredits] = useState("");
  const [creditsState, setCreditsState] = useState("success");
  const [administratorSelect, setAdministratorSelect] = useState("");
  const [selectState, setSelectState] = useState("success");

  useEffect(() => {
    setName(data.name);
    setCredits(data.credits);
    setAdministratorSelect(data.administrator.id);
  }, [data]);
  ...
  const onSubmithandle = event => {
    ...
    if (!verifyLength(name, 0)) {
      setNameState("error");
    }
    if (verifyLength(credits)) {
      setCreditsState("error");
    }
    if (!verifyLength(administratorSelect, 0)) {
