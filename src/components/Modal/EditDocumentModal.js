...
const EditDocumentModal = ({ open, onClose, data }: Props) => {
  const { id } = useParams();
  const classes = useStyles();
  //Form State
  const [country, setCountry] = useState("");
  const [countryState, setCountryState] = useState("success");
  const [docNo, setDocNo] = useState("");
  const [docNoState, setDocNoState] = useState("success");
  const [issueDate, setIssueDate] = useState("");
  const [expiryDate, setExpiryDate] = useState("");
  const [docType, setDocType] = useState("");
  const [docTypeState, setDocTypeState] = useState("success");
  ...
  useEffect(() => {
    if (data.issue_date != "") {
      setDocNo(data.document_number);
      setCountry(data.country);
      setIssueDate(data.issue_date.toDate());
      setExpiryDate(data.expiry_date.toDate());
